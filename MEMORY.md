...
- Five tests fail on a clean tree through the end of M2 step 2: three in `test/camera.test.js`, two in `test/kick.test.js`. They fail at the last M1 commit too, so all-green-except-five is the baseline, not a regression.
