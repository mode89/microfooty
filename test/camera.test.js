...
test('the view shows the agreed fraction of the pitch width', () => {
  ...
  assert.ok(Math.abs(visibleWidth / PITCH.width - VISIBLE_PITCH_WIDTH_FRACTION) < 1e-9);
});
...
test('the camera keeps the view inside the pitch plus its margin', () => {
  ...
  assert.ok(clamped.centre.x + view.halfWidth <= PITCH_BOUNDS.maxX + CAMERA.boundsMargin + 1e-9);
  assert.ok(clamped.centre.y + view.halfHeight <= PITCH_BOUNDS.maxY + CAMERA.boundsMargin + 1e-9);
});
...
test('the pitch is vertical, so it is longer along y than along x', () => {
  assert.ok(PITCH_BOUNDS.maxY - PITCH_BOUNDS.minY > PITCH_BOUNDS.maxX - PITCH_BOUNDS.minX);
});

test('each penalty arc ends on the edge of its penalty area', () => {
  const boxEdge = PITCH_BOUNDS.maxY - PITCH.penaltyAreaDepth;
  const arcs = PITCH_MARKINGS.filter((m) => m.kind === 'arc' && m.radius === PITCH.centreCircleRadius);
